 ...
 * Includes all static pages, dynamic blog posts, book pages, and quiz pages.
 */
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // Dynamic quiz pages (one per published book)
  ...
  try {
    ...
    quizPages = books.map((book) => ({
      url: `${baseUrl}/quiz/${book.slug}`,
      lastModified: new Date("2026-03-04"),
      changeFrequency: "monthly" as const,
      priority: 0.7,
    }));
  } catch {
    // Fallback: known quiz slugs
    quizPages = ["the-exiles-return", "the-warrior-prince"].map((slug) => ({
  ...
  return [...staticPages, ...blogPages, ...bookPages, ...quizPages];
}
